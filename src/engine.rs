...
fn abeta(
    ...
    //let mut hash_pos = get_tte(g, encoded_board);

    let hash_pos = if cup == 0 && g.last_abort_depth == v_depth as u8 + 8 {
        //println!("**********");
        -1
    } else {
        get_tte(g, encoded_board)
    };
    if hash_pos >= 0 {
    ...
    */
    ...
    for el in &mut hash_res.kks {
        ...
        if cup == 0 {
            if (g.start_time.elapsed() > g.time_3)
                || (eval_cnt > 1 && g.start_time.elapsed() > g.time_2)
            {
                ...
                debug_assert!(eval_cnt as usize + 1 == hash_res_kks_high);
                if false && cfg!(feature = "salewskiChessDebug") {
                    println!("{:?}", hash_res.kks);
                }
                g.abort_depth = v_depth as u8;
                break;
            }
        }
...
pub fn do_move(g: &mut Game, p0: Position, p1: Position, silent: bool) -> i32 {
    ...
    if !silent {
        g.last_abort_depth = g.abort_depth;
        g.abort_depth = 255;
    }
    result
}
...
pub fn reply(g: &mut Game) -> Move {
    let color = ((g.move_counter as i64 + 1) % 2) * 2 - 1; // as i64;
                                                           //println!("COL {}", color);
